...
static void calculate_scaling_params(const struct Config *config, ScalingParams *params, const char *device_name)
{
    ...
    else if (strcmp(config->display_shape, "circular") == 0)
    {
        ...
        log_message(LOG_INFO, "Circle mode: Display shape forced to circular via config (inscribe_factor: %.4f)", params->inscribe_factor);
    }
    ...
    log_message(LOG_INFO, "Circle mode scaling: safe_area=%.0fpx, bar_width=%dpx (%.0f%%), margin=%.1fpx",
...
/**
 * @brief Draw single sensor display (CPU or GPU) with optional liquid temperature
 */
static void draw_single_sensor(cairo_t *cr, const struct Config *config, const ScalingParams *params,
                               float temp_value, float temp_liquid, SensorMode sensor)
{
    if (!cr || !config || !params)
        return;
    ...
    if (sensor == SENSOR_CPU)
    {
        ...
        set_cairo_color(cr, &config->layout_bar_color_background);
        draw_rounded_rectangle_path(cr, bar_x, liquid_bar_y, effective_bar_width, bar_height, params->corner_radius);
        cairo_fill(cr);
        ...
        const int liquid_fill_width = calculate_temp_fill_width(temp_liquid, effective_bar_width, max_liquid_temp);
        ...
        // Liquid temperature text (under liquid bar with 1 decimal, half size, 10% spacing)
        char liquid_temp_str[16];
        snprintf(liquid_temp_str, sizeof(liquid_temp_str), "%.1f", temp_liquid);

        const float liquid_font_size = config->font_size_temp / 2.0f; // Half size of normal temp
        ...
        const double liquid_total_width = liquid_temp_ext.width - 4 + liquid_degree_ext.width;
        ...
        const double liquid_degree_x = liquid_temp_x + liquid_temp_ext.width + degree_spacing / 2.0;
        const double liquid_degree_y = liquid_temp_y - liquid_font_size * 0.25;
        cairo_set_font_size(cr, liquid_font_size / 1.66); // Match liquid temp sizing
                                   ...
                                   const monitor_sensor_data_t *data, const ScalingParams *params)
{
    ...
    draw_single_sensor(cr, config, params, temp_value, data->temp_liquid, current_sensor);
}
