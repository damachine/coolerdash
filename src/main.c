...
static const char *read_version_from_file(void)
{
    ...
    // Try to read from VERSION file with security checks
    struct stat version_stat;
    FILE *fp = NULL;

    // Check VERSION file in current directory first
    ...
    if (!fp)
    {
        ...
        if (stat("/opt/coolerdash/VERSION", &version_stat) == 0 && S_ISREG(version_stat.st_mode))
        {
...
static int check_existing_instance_and_handle(const char *pid_file, int is_service_start)
{
    ...
    if (!fp)
        return 0; // Could not open PID file
...
static int write_pid_file(const char *pid_file)
{
    ...
    // First, check if the target is a symlink to prevent symlink attacks
    struct stat link_stat;
    if (lstat(temp_file, &link_stat) == 0)
    {
        if (S_ISLNK(link_stat.st_mode))
        {
            log_message(LOG_ERROR, "Symlink attack detected for PID file '%s'", temp_file);
            return -1;
        }
        // File already exists, remove it if it's a regular file
        if (S_ISREG(link_stat.st_mode))
        {
            if (unlink(temp_file) == -1)
            {
                log_message(LOG_ERROR, "Could not remove existing temporary PID file '%s': %s", temp_file, strerror(errno));
                return -1;
            }
        }
        else
        {
            log_message(LOG_ERROR, "Temporary PID file path '%s' exists but is not a regular file", temp_file);
            return -1;
        }
    }
...
static void send_shutdown_image_if_needed(void)
{
    ...
    // Check if shutdown image file exists and is a regular file (not a symlink or device)
    struct stat file_stat;
    if (stat(shutdown_image_path, &file_stat) == 0 && S_ISREG(file_stat.st_mode))
    {
        // Image exists and is a regular file, send it normally
