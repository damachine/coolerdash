...
static void draw_single_temperature_bar(cairo_t *cr, const struct Config *config, float temp_value, int bar_x, int bar_y)
{
    ...
    const int safe_temp_val_w = (temp_val_w < 0) ? 0 : (temp_val_w > config->layout_bar_width) ? config->layout_bar_width
                                                                                               : temp_val_w;
...
static void draw_temperature_bars(cairo_t *cr, const monitor_sensor_data_t *data, const struct Config *config)
{
    ...
    if (config->layout_bar_width > config->display_width)
        return; // Safety check
    ...
    if (total_bar_height > config->display_height)
        return; // Safety check
 ...
 */
/**
 * @brief Setup cairo surface and context for rendering.
 * @details Helper function to create and validate cairo surface and context.
 */
static cairo_t *setup_cairo_context(const struct Config *config, cairo_surface_t **surface)
{
    if (!config || !surface)
        return NULL;
...
/**
 * @brief Configure cairo font and color settings.
 * @details Helper function to setup font and color configuration.
 */
static void configure_cairo_fonts(cairo_t *cr, const struct Config *config)
{
    if (!cr || !config)
        return;
...
/**
 * @brief Setup label fonts if conditions are met.
 * @details Helper function to configure label font and color if temperatures are below 99°C.
 */
static void setup_label_fonts_if_needed(cairo_t *cr, const struct Config *config,
...
/**
 * @brief Validate cairo context and save PNG.
 * @details Helper function to check for errors and save the rendered image.
 */
static int validate_and_save_png(cairo_t *cr, cairo_surface_t *surface, const struct Config *config)
{
    if (!cr || !surface || !config)
        return 0;
...
int render_display(const struct Config *config, const monitor_sensor_data_t *data)
{
    ...
    cairo_t *cr = setup_cairo_context(config, &surface);
    if (!cr)
    {
        return 0;
    }
    ...
    configure_cairo_fonts(cr, config);

    // Render temperature displays and bars
    draw_temperature_displays(cr, data, config);
    draw_temperature_bars(cr, data, config);

    // Setup and render labels if needed
    setup_label_fonts_if_needed(cr, config, data);

    // Validate and save PNG
    int success = validate_and_save_png(cr, surface, config);
...
/**
 * @brief Get sensor data with validation.
 * @details Helper function to retrieve and validate temperature sensor data.
 */
static int get_validated_sensor_data(const struct Config *config, monitor_sensor_data_t *sensor_data)
{
    if (!config || !sensor_data)
        return 0;

    // Initialize data structure with safe defaults
    sensor_data->temp_cpu = 0.0f;
    sensor_data->temp_gpu = 0.0f;
...
/**
 * @brief Get and validate LCD device information.
 * @details Helper function to retrieve device UID, name, and dimensions.
 */
static int get_validated_device_info(const struct Config *config, char *device_uid, size_t uid_size,
                                     char *device_name, size_t name_size,
                                     int *screen_width, int *screen_height)
{
    if (!config || !device_uid || !device_name)
        return 0;
    ...
    const bool device_available = get_liquidctl_data(config, device_uid, uid_size,
...
/**
 * @brief Send image to LCD device with validation.
 * @details Helper function to handle LCD image upload with proper validation.
 */
static void send_image_to_device(const struct Config *config, const char *device_uid, const char *device_name)
{
    if (!config || !device_uid || !device_name)
        return;
    ...
    {
        log_message(LOG_WARNING, "Skipping LCD upload - session not initialized");
        return;
    }
...
void draw_combined_image(const struct Config *config)
{
    ...
    monitor_sensor_data_t sensor_data;
    if (!get_validated_sensor_data(config, &sensor_data))
    {
    ...
    const int valid_device = get_validated_device_info(config, device_uid, sizeof(device_uid),
    ...
    // Send to LCD if device is available
    if (valid_device)
    {
        send_image_to_device(config, device_uid, device_name);
    }
}