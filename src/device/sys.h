 ...
 * parameters. These values are always available and serve as baseline when user
...
typedef struct Config {
  ...
  float display_refresh_interval; // Refresh interval in seconds (e.g., 2.50
                                  // = 2.5 seconds)
  uint8_t lcd_brightness;
  uint8_t lcd_orientation;
  // Developer/testing override: force display to be treated as circular (1) or
  // not (0)
  ...
  // Content scale factor (0.0-1.0) - how much of safe area to use - default:
  // 0.98
  ...
  // Layout configuration - all positioning is calculated dynamically from
  // display dimensions
  uint16_t layout_bar_height;
  uint16_t layout_bar_gap;
  float layout_bar_border;
  uint8_t layout_bar_width; // Bar width as percentage of display width (1-100),
                            // default: 98
  uint8_t layout_label_margin_left; // Label distance from left screen edge
                                    // (1-50%, default: 1)
  uint8_t
      layout_label_margin_bar; // Label distance from bars (1-20%, default: 1)
  ...
  int display_degree_spacing; // Spacing between temperature and degree symbol
                              // (pixels, default: 16)
  ...
  float temp_liquid_max_scale;   // Maximum liquid temperature for bar scaling
                                 // (default: 50°C)
  ...
  Color temp_liquid_threshold_2_bar; // Bar color for threshold_1 <= liquid <
                                     // threshold_2
  Color temp_liquid_threshold_3_bar; // Bar color for threshold_2 <= liquid <
                                     // threshold_3
 ...
 * @details External variable controlling verbose logging behavior across all
 * modules.
 */
 ...
 *          This function always succeeds and provides a valid baseline
 * configuration.
 */
 ...
 * @details Sets fallback values only for fields that are zero/empty (not
 * configured by user). This allows user configuration to take precedence while
 * providing safe defaults.
 *
 *          Call order: 1) init_system_defaults() 2) load user config 3)
